function [X, labels, sizes] = generate_LR_sub_Dataset_for_kNN_with_DCT(X_train, y_train, init_X, max_iter, w, h, w_factor, h_factor)
    ...
    if rho < max_rho
        ...
        if rho <= sigma
            for i=1:size(S,1)
                ...
                O = y_S(i);
                new = getImgFromDCTCoefs(X_new(:, i) + lambda_rho*sign(J(:,O+1,i)), w, h, w_factor, h_factor);
